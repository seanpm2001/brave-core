...
constexpr base::TimeDelta kDebugFetchInterval = base::Minutes(5);

constexpr base::TimeDelta kRetryAfter = base::Minutes(1);

base::TimeDelta GetFetchInterval() {
  return base::Minutes(brave_l10n::kSubdivisionFetchIntervalMinutes.Get());
}
...
void SubdivisionUrlRequest::FetchAfterDelay() {
  const base::Time fetch_at = timer_.StartWithPrivacy(
      FROM_HERE, ShouldDebug() ? kDebugFetchInterval : GetFetchInterval(),
